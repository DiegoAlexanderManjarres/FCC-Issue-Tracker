...
const validateInputFields = (inputs, errorsArray = []) => {
   for (let property in inputs) { inputs[property] = inputs[property].trim() }
   const { issue_title, issue_text, created_by, assigned_to, status_text } = inputs
...
const validateInputFieldsUpdate = (inputs, errorsArray = []) => {
   ...
   const { _id, issue_title, issue_text, created_by, assigned_to, status_text } = inputs

   if (_id && _id.length !== 24) { errorsArray.push('invalid id') }
   ...
   app.route('/api/issues/:project')
      .get(async (req, res) => {
         try {  
            ...
            const queryFilters = /^(_id|open|created_by|assigned_to|issue_title|status_text)$/
         ...
         } catch(err) { res.status(500).json('database err') }   
      })
      ...
      .post(async (req, res) => {
         try {
            ...
            const result = await db().collection('issues_tracker').insertOne(newIssue)
      ...
      .put(async (req, res) => {
         try {
            ...
            ifFieldsValidationFails(validationErrors)
            const  { _id } = req.body
            const issue = await db().collection('issues_tracker')
               .findOne({ _id: new ObjectId(_id) })
            if (!issue) { return res.status(200).type('text').send('no updated field sent') }
           ...
           if (req.body.hasOwnProperty('open')) {
            ...
            const updateResult = await db().collection('issues_tracker')
               .findOneAndUpdate( 
      ...
      .delete(async (req, res) => {
         ...
         if (_id.length !== 24) { return res.status(200).type('text').send('_id error') }

         const result = await db().collection('issues_tracker')
            .findOneAndDelete({ _id: new ObjectId(_id) })
         if (!result.value) { 
            return res.status(200).type('text').send('Unable to delete issue') 
         }

         return res.status(200).type('text').send('deleted ' + req.body._id)  
         } catch(err) {
           return res.status(200).type('text').send(`could not delete ${req.body._id}`)
         }  
      })
